...
def preprocess(image):
	...
	# Take Y channel and normalize image
	img_yuv = cv2.cvtColor(resized_image, cv2.COLOR_BGR2YUV)
	img_y = img_yuv[:, :, 1]
	img_y = np.float32(img_y)
	#img_y = (img_y - np.mean(img_y)) / np.mean(img_y)
	img_y = (img_y - 128.0) / 128.0
...
data_file = '/home/linfeng-zc/Documents/Udacity/CarND-Behavioral-Cloning/data/data/driving_log.csv'
image_path = '/home/linfeng-zc/Documents/Udacity/CarND-Behavioral-Cloning/data/data/'
num_epoch = 10
batch_size = 32
...
model.fit_generator(get_generator(train_lines, image_path, batch_size), 
	nb_epoch=num_epoch, samples_per_epoch=len(train_lines), callbacks=None)

#model.fit(preprocessed_img1[None, :, :, :], np.array([0.0617599]), nb_epoch=num_epoch, batch_size=batch_size, verbose=2)
